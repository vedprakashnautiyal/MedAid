...
app = FastAPI(title="MedAid", version="1.0")
...
def chat(
    ...
    try:
        ...
        if not messages:
            raise HTTPException(status_code=500, detail="Empty message list from agent")

        last_message = messages[-1]
        if isinstance(last_message, dict):
            response_text = last_message.get("content", "No content")
        else:
            response_text = getattr(last_message, "content", "No content")
